...
# create a dummy run_healthcheck
...
def parse_args():
    ...
    parser.add_argument("--num_test_episodes", type=int, default=1500, help="Number of testing episodes")
...
num_episodes_test = args.num_test_episodes
...
class BasicEnv(Env):
    ...
    def __init__(self, dataset, order, train_fraction, MDP_model):
        ...
        if self.dataset == 'SimBank':
            if self.MDP_model == 'HMDP': #removing control flow variables that make the process MDP
        ...
        #self.state_cols.append('last_action_encode')
        ...
        for col in self.categorical_cols:
            ...
            if n_unique <= 40:
                ...
                reshaped = self.train_df[[col]]  # needs 2D array
                ...
                self.onehot_encoders[col] = ohe  # save for transforming new data
        ...
        if self.order > 1:
            ...
            for orig_col in self.dataset_manager.dynamic_cat_cols:
                ...
                elif orig_col in self.train_df.columns:  # embedding categorical
                ...
                for col in cols_to_lag:
                    for lag in range(1, order):
                        ...
                        lagged_dict[lagged_col] = (
                            df.groupby("ID")[col]
                            .shift(lag)
                            .fillna(0)  # or np.nan depending on your preference
                        )
            ...
            # Apply to test using the same lagged columns
        ...
        '''
        self.all_states = self.df[self.state_cols].drop_duplicates().reset_index(drop=True)
        self.full_state = np.array(self.all_states.iloc[self.state])
        self.state = np.array(self.state, dtype=np.int16)
        self.action_space = spaces.Discrete(self.n_actions) 

        self.transition_proba = transition_probabilities_faster(self.df, self.state_cols, self.all_states , self.activity_index, self.n_actions)
        ...
        actions_to_remove = ["start", "archive_application"]
        # Now build valid traces using indices, excluding unwanted actions
        self.valid_traces = [
            tuple(a for a in x if a not in actions_to_remove)
            for x in self.df.groupby('ID')['action'].apply(list).drop_duplicates()
        ]

        self.illegal_trace_total = 0
        self.intermediate_penalty = intermediate_penalty'''
    ...
    def step(self, action):
            ...
            granted = False
            previous_state = self.state.item()
            transition = self.transition_proba.get((previous_state, action), None)
            ...
            self.full_state = np.array(self.all_states.iloc[self.state], dtype=np.float32)
            #self.state = np.array(next_state, dtype=np.int32)
            ...
            done, granted = self.config.is_terminal_successful_state(self.all_states.iloc[self.state].to_dict())
            
            # invalid trace check
            if not any(valid_trace[:len(self.current_trace)] == tuple(self.current_trace) for valid_trace in self.valid_traces):
                    info["invalid_trace"] = True
                    self.illegal_action_count += 1
                    self.current_trace_illegal = True
                   
            # terminal actions
            if done:
                self.unscaled_outcome = np.float32(
                self.config.outcome_function(granted,
                                             self.all_states.iloc[self.state].to_dict(),
                                             self.cum_costs,
                                             self.scaler,
                                             self.numeric_cols,
                                             self.state_cols))
                self.reward = self.unscaled_outcome /self.reward_scale

            else:
                self.reward = 0.0  # intermediate action
                self.unscaled_outcome = 0.0
                
            return self.full_state, self.reward, self.unscaled_outcome, done, granted, info
    ...
    def reset(self, case):
        ...
        self.case = case
        self.state = self.all_state_index[tuple(self.case.iloc[0][self.state_cols])] #retrieve an inital state
        self.full_state = np.array(self.all_states.iloc[self.state], dtype=np.float32)
        self.state = np.array(self.state, dtype=np.int32)
        self.possible_actions = [action for action in range(self.n_actions)
                                if (self.state.item(), action) in self.transition_proba and self.transition_proba[(self.state.item(), action)].sum() > 0]
        return self.full_state
    

    
    def build_state_with_lags(self, previous_trace, current_event):
        """
        ...
        #state_dict['last_action']=state_dict['activity']
            ...
            # Add skip/contact flags for MDP
        if self.MDP_model == 'MDP':
            skip_done = any(e["activity"] == "skip_contact" for e in previous_trace) or current_event["activity"] == "skip_contact"
            contact_done = any(e["activity"] == "contact_headquarters" for e in previous_trace) or current_event["activity"] == "contact_headquarters"
            state_dict["skip"] = 1 if skip_done else 0
            state_dict["contact_hq"] = 1 if contact_done else 0

        # Apply one-hot encoding for categorical columns
        for col in self.onehot_cols:
            ohe = self.onehot_encoders[col]  # get the fitted encoder
            val = str(state_dict.get(col))  # fallback for unseen category
            value = np.array([[val]], dtype=object)  # 2D array for sklearn
            onehot_values = ohe.transform(value).flatten()  # 1D array of one-hot
            # overwrite state_dict with the one-hot columns
            for i, cat in enumerate(ohe.categories_[0]):
                state_dict[f"{col}_{cat}"] = onehot_values[i]
            # optionally remove original column if still in state_dict
        ...
        # Convert to numpy row in the correct state order
        state_row = [state_dict[col] for col in self.state_cols]
        current_state = np.array(state_row, dtype=np.float32).reshape(1, -1)  # batch dimension
...
train_df = env.train_df.copy()  # make a copy to avoid modifying original
...
for case_id, case_df in train_df.groupby("ID"):
    # Iterate through rows/events of this case
    len_case = len(case_df)
    for idx in range(0, len_case-1):
        # Example: get reward and terminal
...
test_df = env.test_df.copy()  # make a copy to avoid modifying original
...
#algo.save_model(f'trained_algo/{args.dataset}_{method}_{MDP_model}_order_{order}_N_{num_steps}_batch_{BATCH_SIZE}.pt')

algo.load_model(f'trained_algo/{args.dataset}_{method}_{MDP_model}_order_{order}_N_{num_steps}_batch_{args.batch_size}_nhidden1_{args.n_hidden1}_nhidden2_{args.n_hidden2}_deep_{args.deep_network}_target_{target_update}.pt')
...
results_fqe = fqe.fit(
    ...
    save_interval=200,
    experiment_name=f'{args.dataset}_{method}_{MDP_model}_{order}_N_{num_steps}_batch_{args.batch_size}_nhidden1_{args.n_hidden1}_nhidden2_{args.n_hidden2}_deep_{args.deep_network}_target_{target_update}_NOPE_{args.num_steps_ope}_encoding_final',
    evaluators={
...
with open(f"results_real/{args.dataset}_{method}_{MDP_model}_order_{order}_N_{num_steps}_batch_{args.batch_size}_nhidden1_{args.n_hidden1}_nhidden2_{args.n_hidden2}_deep_{args.deep_network}_target_{target_update}_NOPE_{args.num_steps_ope}_encoding_final{endfile}_testing.pkl", "wb") as f:
...
if dataset == 'SimBank': 
    ...
    os.makedirs("results_real", exist_ok=True)
    with open(f"results_real/{args.dataset}_{method}_{MDP_model}_order_{order}_N_{num_steps}_batch_{BATCH_SIZE}_{endfile}_testing.pkl", "wb") as f:
            pickle.dump(test_results, f)
    print("saved")

os.makedirs("trained_fqe", exist_ok=True)
fqe.save_model(f'trained_fqe/{args.dataset}_{method}_{MDP_model}_order_{order}_N_{args.num_steps_ope}_batch_{args.batch_size}__nhidden1_{args.n_hidden1}_nhidden2_{args.n_hidden2}_deep_{args.deep_network}_target_{target_update}_encoding_final.pt')
